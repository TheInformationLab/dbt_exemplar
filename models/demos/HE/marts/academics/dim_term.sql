...
with terms_from_enrolments as (
    ...
    from {{ ref('stg_csv_dump__course_enrolments') }}
)
...
, enriched as (
    ...
    select
        ...
        , cast (split_part(academic_term, '/', 1) as int) as academic_year_start
        , cast (('20' || split_part(split_part(academic_term, '/', 2), ' ', 1)) as int)
        ...
        , case
            when
                academic_term ilike '%S1%'
                then cast (split_part(academic_term, '/', 1) as int)
            else cast (split_part(academic_term, '/', 1) as int) + 1
